...
public class ConnectionManager implements Runnable {
   ...
   private Connection CreateNewConnection(final SocketChannel sockChan) {
      ...
      String s = "Connection:"
            + Integer.toString(this.getNewConnectionNumber());
   ...
   private boolean disconnect(final SelectionKey key) throws IOException {
      System.out.println("ConnectionManager.disconnect(SelectionKey): key="
            + key.toString());
   ...
   private boolean disconnect(final SocketChannel sockChan) throws IOException {
      System.err
            .println("ConnectionManager.disconnect(SocketChannel): sockChan="
                  + sockChan.toString());
   ...
   public final boolean disconnectFrom(final Connection c) throws IOException {
      System.out.println("ConnectionManager.disconnect(Connection): c="
            + c.toString());
   ...
   public final void init(final InetAddress hostAddress, final int port)
         throws IOException {
      // Init the Selector:
      ...
      while (this.runCmd) {
         try {
            ...
            synchronized (this.pendingEvents) {
               ...
               for (ConnEvent pendingEvent : this.pendingEvents) {
                  ...
                  case ConnEvent.REGISTER:
                     pendingEvent.socket.register(this.selector,
                           pendingEvent.ops);
                     break;
                  default:
                     break;
            ...
            try {
               ...
               System.err
                     .println("During CancelledKeyException, a key failed to disconnect.");
            }
         } catch (Exception e) {
   ...
   public final void send(final SocketChannel sockChan, final byte[] data) {
      synchronized (this.pendingEvents) {
         ...
         this.pendingEvents.add(new ConnEvent(sockChan, ConnEvent.CHANGEOPS,
   ...
   /**
    * This method is called by the run() method after the selector is unblocked.
    * This method performs the actual data write to the SocketChannel. Also, the
    * associated Key is set back to OP_READ so we don't waste CPU cycles while
    * it waits for more data to write when there isn't any coming!
    * @param key
