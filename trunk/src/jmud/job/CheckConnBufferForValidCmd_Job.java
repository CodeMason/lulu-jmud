...
public class CheckConnBufferForValidCmd_Job extends AbstractJob {
	...
	public CheckConnBufferForValidCmd_Job(Connection c) {
		super();
		this.c = c;
	}
	...
	public boolean doJob() {
		...
		synchronized (this.c.getReadBuffer()) {
			try {
				...
				System.err.println("CheckConnBufferForValidCmd_Job.doJob(): CharacterCodingException in ByteBuffer of: "
								+ this.c.getSc().socket().getInetAddress().toString());
				this.c.getReadBuffer().clear();
				// TODO Send a Message to the client telling them a server error occured.
				return false;
			}
		...
		System.out.println(this.c.getSc().socket().getInetAddress().toString() + ": New Command String Recvd: " + cmd);
